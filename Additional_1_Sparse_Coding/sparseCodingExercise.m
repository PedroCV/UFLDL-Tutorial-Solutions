...
lambda = 5e-2;  % L1-regularisation parameter (on features)
...
%     mu = 2/lambda;
%     for itt = 1:1
%         aux = featureMatrix - ((gam_reg*mu).*(weightMatrix')*((weightMatrix*featureMatrix)-batchPatches));
%         featureMatrix = sign(aux).*(max(0,abs(aux)-gam_reg));
%     end
% % %==   
